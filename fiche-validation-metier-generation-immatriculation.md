...
> acteurs métier) · `soutenance-strategie-generation.md` (argumentaire : chaque décision avec ses
> alternatives écartées et ses preuves — pour la revue avec le lead technique) ·
> `releve-bascule-valeurs-initiales.md` (valeurs initiales de bascule, généré par script).
>
> **Stratégie VALIDÉE PAR SIMULATION** le 22/07/2026 : 20 788 attributions, 53/53 contrôles OK —
> prototype fidèle, campagne et rapport dans `simulation/` (cf. §18).
...
**Seconde contrainte UNIQUE — `dossier_id`** : deux traitements simultanés du même dossier
(double clic, deux nœuds traitant le même lot rejoué) passent tous deux le contrôle d'idempotence
avant d'allouer → sans contrainte, double attribution. La contrainte
`UNIQUE (dossier_id)` (index partiel `WHERE dossier_id IS NOT NULL`) ferme cette course : la
seconde transaction échoue sur l'index et **relit la plaque déjà attribuée** (même sémantique
qu'un rejeu). Faille détectée à la conception du simulateur, comportement validé en campagne.
...
- **Mode de bascule retenu (décision projet)** : PAS d'import massif de l'historique. À la bascule,
  l'**ancien système est ARRÊTÉ**, les **derniers numéros de chaque compteur sont relevés**
  (fichier de calage + PV, cf. §10) et configurés dans SIGATT ; la génération **continue à partir
  de ces derniers numéros**. L'arrêt préalable rend le relevé exact (plus de problème de fraîcheur
  d'extraction) ; une marge de sécurité paramétrable reste appliquée par prudence.
- **Conséquence assumée** : le registre démarre vide (il ne contient plus l'historique) — la couche 3
...
## 10. Bascule : arrêt de l'ancien système et calage des compteurs (sans import massif)

**Mode de bascule retenu (décision projet, 22/07/2026) : PAS d'import massif de l'historique.**
L'ancien système est arrêté, les derniers numéros sont relevés et configurés dans SIGATT, la
génération continue à partir d'eux. Déroulé :

1. **Arrêt de l'ancien système** : plus aucune délivrance de numéro — le relevé qui suit est exact
   au moment T (l'ancien problème de fraîcheur des extractions disparaît).
2. **Relevé des derniers numéros = le FICHIER DE CALAGE** (petit CSV, une ligne par compteur),
   produit par **requête SQL sur l'ancien système** — jamais à la main. Il doit couvrir :
   ...
   critique de la bascule — sans historique dans le registre, une erreur de relevé n'est pas
   rattrapable par la couche 3.
   → **Inventaire exhaustif des valeurs à relever + valeurs de référence d'avril 2024 + contrôles
   de cohérence : `releve-bascule-valeurs-initiales.md`** (généré par
   `simulation/genere_releve_reference.py`, à régénérer au jour J). Il inclut les **24 plaques
   `CMD` existantes** — seul élément à insérer au REGISTRE (pas un compteur), indispensable pour
   faire respecter « un seul véhicule par chef de mission » sans import de l'historique.
3. **Configuration dans SIGATT** : chargement du fichier de calage dans `serie_immatriculation`
   (job d'import du CSV ou écran d'administration des compteurs — livrable à prévoir), avec la
   **marge de sécurité** paramétrable (`immatriculation.amorcage.marge`, défaut proposé : 10 —
   elle peut être faible puisque l'ancien système est arrêté, mais jamais nulle).
4. **Motos — la décision D4 reste entière** : les stocks de plaques pré-imprimées existent
   indépendamment du mode de bascule. « Reprendre au dernier numéro » (`DH1` @ relevé) exposerait
   toujours aux **7,7 % de conflits physiques mesurés en simulation** (§18) ; la recommandation
   demeure `PRIVE|CYCLE = (DJ1, 0)` — qui est aussi une « continuité » : le groupe suivant de
   l'alphabet réglementaire, jamais mis en fabrication.
5. **Activation gardée** par `immatriculation.generation.active`, désactivable sans redéploiement.
6. **Import de l'historique = OPTION DIFFÉRÉE, non bloquante** : la génération n'en dépend plus.
   Il reste souhaitable plus tard pour (a) vérifier l'existence des plaques anciennes saisies en
   mutation/renouvellement, (b) offrir la recherche historique dans SIGATT. Sans lui, une plaque
   précédente saisie n'est contrôlable que par son **format** (§11) — elle entre au registre au
   fil de l'eau et y devient protégée.
...
|---|---|
| Privés (lettres simples **et croisées** — correction issue de la simulation : la version initiale oubliait le croisement à deux lettres, déjà actif côté motos) | `^\d{4} [DEFGHJKLMNQRSUVXYZ]{1,2}[1-9] (0[1-9]\|1[0-3])( (IT\|AT))?$` |
| État / Collectivités / Parapublics / Police | `^\d{4} [ABCP][1-9] (0[1-9]\|1[0-3])( (IT\|AT))?$` |
| Transporteurs publics (chiffre 1 à 99) | `^\d{4} T[1-9]\d? (0[1-9]\|1[0-3])( (IT\|AT))?$` |
...
| Cycles | mêmes règles avec groupe inversé (chiffre(s) devant), ex. `^\d{4} [1-9][DEFGHJKLMNQRSUVXYZ]{1,2} (0[1-9]\|1[0-3])( (IT\|AT))?$` (transport cycle : `[1-9]\d?T`) |

Implémentation de référence : `simulation/simulateur_immatriculation.py` (`parser_plaque` +
`_groupe_ordinaire_valide`) — validée par la campagne (0 format invalide sur 20 788 numéros
générés, rejet vérifié des lettres interdites `I`/`O`).
...
**Règle d'or : ne jamais saisir ces valeurs à la main.** Elles proviennent du **fichier de calage**
relevé par requête SQL à l'arrêt de l'ancien système (cf. §10 — mode de bascule sans import massif),
chargé dans `serie_immatriculation` par un job d'import du CSV ou un écran d'administration des
compteurs (livrable à prévoir). Indispensable notamment pour créer **une ligne par mission
diplomatique × bande** (163 compteurs dénombrés — sans quoi le premier tirage `CD` repartirait de
0001, en plein parc existant que le registre, désormais vide au départ, ne connaît pas).

Valeurs **effectivement produites par le job de calage simulé** sur les extractions d'avril 2024
(campagne du 22/07/2026 — recalage obligatoire à la bascule) :

| Compteur | Front observé | | Compteur | Front observé |
|---|---|---|---|---|
| `PRIVE\|VEHICULE` | (G3, 2088) | | `PRIVE\|CYCLE` | (DH1, 6615) — ou **(DJ1, 0)** recommandé |
| `ETAT\|VEHICULE` | (A1, 4237) | | `ETAT\|CYCLE` | **(A1, 8746) — série à 87 %, à surveiller** |
| `COLLECT\|VEHICULE` | (B1, 357) | | `COLLECT\|CYCLE` | (B1, 3621) |
| `PARAPUB\|VEHICULE` | (C1, 1390) | | `PARAPUB\|CYCLE` | (C1, 355) |
| `POLICE\|VEHICULE` | (P1, 453) | | `POLICE\|CYCLE` | (P1, 661) |
| `TRANSPORT\|VEHICULE` | (T1, 8761) | | `TRANSPORT\|CYCLE` | (T1, 96) |
| `CD\|SERVICE\|01` | (CD, 35) | | `CD\|PERSONNEL\|01` | (CD, 1022) |
| `IN\|SERVICE\|27` | (IN, 38) | | **+ 163 compteurs diplomatiques** dérivés automatiquement |
...
Deux enseignements de la simulation : (1) **163 compteurs diplomatiques** existent — le calage
manuel serait ingérable, la dérivation automatique depuis le registre est obligatoire ;
(2) la série moto de l'État (`1A`) est déjà à **87 % de remplissage** — le passage `A1→A2`
(automatique) surviendra tôt après la bascule : à mentionner au métier.
...
> Build vert (referentiel 127 tests). Reste : afficher le champ dans l'écran front `type-demande`,
> et faire ajuster libellés/montants/délais/`est_public` par le métier.
...
## 18. Résultats de la campagne de simulation (22/07/2026)

La stratégie a été **validée par exécution** d'un prototype Python fidèle au pseudo-code du §7.3
(`simulation/` : moteur + import du legacy réel + campagne ; rapport détaillé :
`simulation/rapport-simulation.md`, graine fixée, résultats reproductibles).

**Bilan : 53/53 verdicts OK — 20 788 attributions simulées** (minimum demandé : 500).

> Note : la campagne a validé la logique en important l'historique complet dans le registre.
> Le mode de bascule finalement retenu (§10 — **calage par relevé, sans import**) ne change RIEN
> à la logique d'allocation validée : seule la provenance des valeurs initiales des compteurs
> diffère (fichier de calage au lieu de dérivation depuis le registre). Les conclusions de V4
> (motos : 7,7 % de conflits en continuité vs 0 en série vierge) restent intégralement valables —
> les stocks pré-imprimés existent indépendamment du mode de bascule.

| Volet | Contenu | Résultat |
|---|---|---|
| V1 | Rejeu automatisé des 44 scénarios S01-S44 (dont fins de série, croisement `Z9→DD1→DE1`, `T9→T10`, bandes pleines, CMD en doublon, saut de collision, idempotence, épuisement des 50 retries) | 44/44 conformes aux valeurs attendues documentées |
| V2 | 12 000 dossiers aléatoires réalistes sur le **legacy réel importé** (1 325 799 plaques, 75 698 rééditions ignorées, 0 ligne illisible) | 11 746 attributions, 0 doublon, 0 format invalide, 0 rupture d'idempotence sur 2 % de rejeux ; ~37 000 attributions/s (logique pure) |
| V3 | Concurrence : 8 threads × 250 sur la même série + 1 000 tirages mixtes multi-séries | 2 000 numéros **parfaitement contigus** (2089..4088), 0 doublon, 0 interférence |
| V4 | Arbitrage moto : 5 000 plaques de stock pré-imprimées posées après bascule, entrelacées avec 3 000 tirages, sur les deux variantes | **Continuité `DH1` : 384 conflits physiques (7,7 % du stock)** — des plaques fabriquées dont le numéro a été redonné à une autre moto ; **`DJ1` : 0 conflit** |

**Améliorations issues de la campagne, intégrées à ce document :**
1. Clé d'unicité du registre = groupe **tel qu'affiché** (distinction voiture/moto) — §5.2 ;
2. Contrainte `UNIQUE (dossier_id)` au registre (course d'idempotence) — §5.2 et couche 4 ;
3. Regex de validation corrigées (croisement deux lettres, `T10-T99`, cycles) — §11 ;
4. Compteurs réels complets révélés par le calage (motos catégorielles, 163 diplomatiques,
   alerte `ETAT|CYCLE` à 87 %) — §15.1 ;
5. Chiffrage de la décision D4 : 7,7 % de conflits physiques en continuité vs 0 en série vierge —
   la recommandation `DJ1` devient quantifiée.
